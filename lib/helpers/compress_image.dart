...
class Luban {
  ...
  static String _imageName(File image){
    return image.path.substring(image.path.lastIndexOf("/")+1, image.path.lastIndexOf("."));
  }
  ...
  static String _lubanCompress(CompressObject object) {
    ...
    if (isJpg)
      decodedImageFile = new File(
          object.path + '/${_imageName(object.imageFile)}.jpg', );
    else if (isPng)
      decodedImageFile = new File(
          object.path + '/${_imageName(object.imageFile)}.png');
    else
      throw Exception("flutter_luban doesn't support this image type");

    if (decodedImageFile.existsSync()) {
      return decodedImageFile.path;
    }

    decodedImageFile.createSync();
